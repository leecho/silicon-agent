...
use silicon_worker::session::task_queue::{
    cap_for, decide_enqueue, drain_decision, head_running, parse_queue, remove_queued,
    serialize_queue, DrainAction, EnqueueOutcome, SessionTaskItem, TaskKind, TaskStatus, MAIN_CAP,
    MEMBER_CAP,
};
...
fn enqueue_decision_idle_busy_overflow() {
    // 空闲（运行锁已释放，is_busy=false）→ 立即提升。
    assert_eq!(decide_enqueue(&[], MAIN_CAP, false), EnqueueOutcome::PromoteNow);
    // 忙（is_busy=true）、未满 → 排队。
    let one = vec![item("a", TaskKind::UserMessage, TaskStatus::Running)];
    assert_eq!(decide_enqueue(&one, MAIN_CAP, true), EnqueueOutcome::Queued);
    // 成员上限 3：忙 + 队列已达 3，再来一个溢出。
    let full = vec![
    ...
    assert_eq!(decide_enqueue(&full, MEMBER_CAP, true), EnqueueOutcome::Overflow);
}
...
/// T91 P2：这些纯存储测试无 RunRegistry，按"持久化 Running 队头"模拟在飞 run 的忙态，
/// 保留各用例原意（首条提升后即忙）。生产路径忙态以 run_registry.is_running 为准。
fn store_is_busy(s: &SessionStore, session_id: &str) -> bool {
    head_running(&parse_queue(s.get_pending_tasks(session_id).unwrap().as_deref()))
}
...
fn enqueue_promotes_when_idle_then_queues_when_busy() {
    ...
    let busy = store_is_busy(&s, "sess");
    let r = enqueue_into_store(&s, "sess", new_item("i1", "first"), false, busy, "2").expect("enq1");
    match r {
    ...
    // 忙：第二条排队。
    let busy = store_is_busy(&s, "sess");
    let r = enqueue_into_store(&s, "sess", new_item("i2", "second"), false, busy, "3").expect("enq2");
...
fn enqueue_overflow_on_main_cap_plus_one() {
    ...
    for n in 0..MAIN_CAP {
        let busy = store_is_busy(&s, "sess");
        let r = enqueue_into_store(&s, "sess", new_item(&format!("i{n}"), "x"), false, busy, "2")
            .expect("enq");
        if n == 0 {
    ...
    let busy = store_is_busy(&s, "sess");
    let r = enqueue_into_store(&s, "sess", new_item("overflow", "x"), false, busy, "2").expect("enq");
...
fn drain_completed_pops_head_and_promotes_next() {
    ...
    s.create_session("sess", "S", "1", false).expect("create");
    let busy = store_is_busy(&s, "sess");
    enqueue_into_store(&s, "sess", new_item("i1", "a"), false, busy, "2").expect("e1");
    let busy = store_is_busy(&s, "sess");
    enqueue_into_store(&s, "sess", new_item("i2", "b"), false, busy, "3").expect("e2");
...
fn drain_failed_user_message_halts_and_holds_rest() {
    ...
    // 之后再有提交：运行锁空闲（无在飞 run）→ 提升残留的更早项 i2（FIFO 恢复排空）。
    let busy = store_is_busy(&s, "sess");
    let r = enqueue_into_store(&s, "sess", new_item("i3", "c"), false, busy, "5").expect("e3");
    match r {
