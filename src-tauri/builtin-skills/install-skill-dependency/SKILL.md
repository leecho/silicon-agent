...
1. **Detect OS type**: Determine macOS, Linux, or Windows. This runs on the user's local machine (no VM/secure workspace).
2. **Survey existing toolchains**: Check installation status and versions of common tools:
...
Detect tools by running `which <tool>` / `command -v <tool>` (and `<tool> --version`) with the `run_command` tool.

Present a summary table of the detected environment to the user.

### Step 2: Scan Skill Dependencies

1. **Locate skills directory** (the platform's managed skills root):
...
**macOS / Linux:**
- This runs on the user's host machine — be cautious. Explain each step's purpose and impact clearly before asking for permission (see Step 5).
