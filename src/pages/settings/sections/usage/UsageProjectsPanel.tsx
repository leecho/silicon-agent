...
/** 设置页「项目」维度排行表。 */
export function UsageProjectsPanel({ data }: { data: UsageAnalyticsView }) {
  const grandTotal = data.totals.total || 1;
  const rows = useMemo(() => [...data.byProject].sort((a, b) => b.total - a.total), [data.byProject]);

  if (rows.length === 0) {
    return <p className="rounded-lg border border-border-subtle bg-surface p-8 text-center text-sm text-foreground-muted">暂无项目维度用量（仅统计归属到项目的会话）。</p>;
  }
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-border-subtle bg-surface">
      <table className="w-full text-left text-sm">
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.projectId} className="border-b border-border-subtle last:border-0">
              <td className="px-3 py-2 font-medium text-foreground">{r.name || r.projectId}</td>
